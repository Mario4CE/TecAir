...
builder.Services.AddDbContext<TecAirDbContext>(options =>
{
...
{
    var db = scope.ServiceProvider.GetRequiredService<TecAirDbContext>();
...
api.MapGet("/usuarios", async (TecAirDbContext db) =>
    Results.Ok(new { usuarios = await db.Usuarios.AsNoTracking().OrderBy(x => x.IdUsuario).ToListAsync() }));

api.MapGet("/usuarios/perfil", async (HttpRequest request, TecAirDbContext db) =>
{
...
api.MapPut("/usuarios/perfil", async (HttpRequest request, UsuarioRequest datos, TecAirDbContext db) =>
{
    var idUsuario = ObtenerIdUsuarioHeader(request);
...
api.MapGet("/usuarios/{idUsuario:int}", async (int idUsuario, TecAirDbContext db) =>
{
    var usuario = await db.Usuarios.AsNoTracking().FirstOrDefaultAsync(x => x.IdUsuario == idUsuario);
    return usuario is null ? Results.NotFound(new { mensaje = "Usuario no encontrado." }) : Results.Ok(new { usuario });
});

api.MapPost("/usuarios", async (UsuarioRequest datos, TecAirDbContext db) =>
{
...
api.MapGet("/aeropuertos", async (TecAirDbContext db) =>
    Results.Ok(new { aeropuertos = await db.Aeropuertos.AsNoTracking().OrderBy(x => x.Nombre).ToListAsync() }));

api.MapPost("/aeropuertos", async (AeropuertoRequest datos, TecAirDbContext db) =>
{
...
api.MapGet("/aviones", async (TecAirDbContext db) =>
    Results.Ok(new { aviones = await db.Aviones.AsNoTracking().OrderBy(x => x.Matricula).ToListAsync() }));

api.MapPost("/aviones", async (AvionRequest datos, TecAirDbContext db) =>
{
...
api.MapGet("/rutas", async (TecAirDbContext db) =>
    Results.Ok(new { rutas = await ObtenerRutasAsync(db) }));

api.MapPost("/rutas", async (RutaRequest datos, TecAirDbContext db) =>
{
...
api.MapGet("/vuelos", async (string? origen, string? destino, TecAirDbContext db) =>
    Results.Ok(new { vuelos = await ObtenerVuelosAsync(db, origen, destino) }));

api.MapGet("/vuelos/{idVuelo:int}", async (int idVuelo, TecAirDbContext db) =>
{
...
api.MapPost("/vuelos", async (VueloRequest datos, TecAirDbContext db, IConfiguration configuration) =>
{
...
api.MapPatch("/vuelos/{idVuelo:int}/abrir", async (int idVuelo, TecAirDbContext db) => await CambiarEstadoVueloAsync(idVuelo, "abierto", db));
api.MapPatch("/vuelos/{idVuelo:int}/cerrar", async (int idVuelo, TecAirDbContext db) => await CambiarEstadoVueloAsync(idVuelo, "cerrado", db));

api.MapGet("/reservaciones", async (HttpRequest request, TecAirDbContext db) =>
{
...
api.MapGet("/reservaciones/{idReservacion:int}", async (int idReservacion, TecAirDbContext db) =>
{
...
api.MapPost("/reservaciones", async (ReservacionRequest datos, TecAirDbContext db, IConfiguration configuration) =>
{
...
api.MapPatch("/reservaciones/{idReservacion:int}/cancelar", async (int idReservacion, TecAirDbContext db) =>
{
...
api.MapGet("/pagos", async (TecAirDbContext db) =>
    Results.Ok(new { pagos = await db.Pagos.AsNoTracking().OrderByDescending(x => x.IdPago).ToListAsync() }));

api.MapPost("/pagos", async (PagoRequest datos, TecAirDbContext db) =>
{
...
api.MapGet("/promociones", async (TecAirDbContext db) =>
    Results.Ok(new { promociones = await ObtenerPromocionesAsync(db) }));

api.MapPost("/promociones", async (PromocionRequest datos, TecAirDbContext db) =>
{
    if (datos.IdRuta <= 0) return Results.BadRequest(new { mensaje = "La ruta es obligatoria." });
...
api.MapGet("/checkins", async (TecAirDbContext db) =>
    Results.Ok(new { checkins = await db.CheckIns.AsNoTracking().OrderByDescending(x => x.IdCheckin).ToListAsync() }));

api.MapGet("/checkins/{idCheckin:int}/pase-abordar", async (int idCheckin, TecAirDbContext db) =>
{
...
api.MapPost("/checkins", async (CheckInRequest datos, TecAirDbContext db) =>
{
    var idUsuario = datos.IdUsuario ?? datos.UsuarioId;
    var idVuelo = datos.IdVuelo ?? datos.VueloId;
    if (idUsuario is null) return Results.BadRequest(new { mensaje = "El usuario es obligatorio." });
...
api.MapGet("/maletas", async (TecAirDbContext db) =>
    Results.Ok(new { maletas = await db.Maletas.AsNoTracking().OrderBy(x => x.IdCheckin).ThenBy(x => x.NumMaleta).ToListAsync() }));

api.MapPost("/maletas", async (MaletaRequest datos, TecAirDbContext db) =>
{
...
static async Task<List<object>> ObtenerRutasAsync(TecAirDbContext db)
{
...
static async Task<object> ObtenerRutaAsync(TecAirDbContext db, int idRuta)
{
...
static async Task<List<VueloResponse>> ObtenerVuelosAsync(TecAirDbContext db, string? origen, string? destino)
{
...
static async Task<object?> ObtenerReservacionAsync(TecAirDbContext db, int idReservacion)
{
...
static async Task<IResult> CambiarEstadoVueloAsync(int idVuelo, string estado, TecAirDbContext db)
{
...
static async Task<List<object>> ObtenerPromocionesAsync(TecAirDbContext db)
{
...
static async Task<object?> ObtenerPaseAbordarAsync(TecAirDbContext db, int idCheckin)
{
...
static async Task<object> ResumenMaletasAsync(TecAirDbContext db, int idCheckin)
{
